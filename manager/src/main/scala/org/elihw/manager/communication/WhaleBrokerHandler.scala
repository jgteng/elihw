...
import org.elihw.manager.mail.{Mail, BrokerHeartMail, RegisterBrokerMail}
 ...
 * Time: 上午10:33
 */
class WhaleBrokerHandler(val connection: TransportConnection_Thread, val brokerRouter: ActorRef, val brokerRecheckTime: Int) extends BrokerServerHandler {
  import Mail._

  val log: Logger = LoggerFactory.getLogger(classOf[WhaleBrokerHandler])
  ...
  def doMsgHandler(message: Message): Message = {
    message.getMsgType match {
      case MessageType.REGISTER_BROKER_REQ => {
        ...
        brokerRouter ! RegisterBrokerMail(registerBrokerReqCmd, this.asInstanceOf[BrokerServerHandler])
      }
